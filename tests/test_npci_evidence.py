...
def test_assistant_context_labels_evidence_derived_after_the_fact():
    ...
    Built from a committed run with the evidence stripped, rather than by
    asserting a particular file has none. The committed runs are regenerated by
    the current pipeline, which does fetch NPCI, so pinning this to a file made
    the test fail every time the book was rebuilt -- and it was never the file
    that mattered, it was the labelling.
    """
    from doctor.assistant import build_context
    from pathlib import Path
    ...
    rec = json.loads((root / "data" / "runs" / "run_beec9668.json")
                     .read_text(encoding="utf-8"))
    rec["report"].pop("external_evidence", None)
...
def test_prove_ignores_runs_that_recorded_no_external_evidence():
    """A run that recorded nothing is reported as recording nothing, and that
    is not a failure. Same reason as above for stripping rather than asserting
    absence: the committed runs now carry evidence, because the pipeline that
    regenerates them fetches it."""
    from pathlib import Path
...
def test_the_committed_runs_carry_evidence_that_verifies():
    """The other half of the pair above, and the one that describes today: the
    book on disk was produced by a pipeline that fetches NPCI, so every run
    should both record evidence and have it verify."""
    from pathlib import Path

    from doctor.prove import verify_external_evidence

    root = Path(__file__).resolve().parents[1]
    runs = sorted((root / "data" / "runs").glob("run_*.json"))
    assert runs, "no committed runs to check"
    for run in runs:
        rec = json.loads(run.read_text(encoding="utf-8"))
        out = verify_external_evidence(rec)
        assert out["ok"], "%s: %s" % (run.stem, out)
